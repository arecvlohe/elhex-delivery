defmodule ElhexDelivery.PostalCode.Navigator do
  ...
  defp do_get_distance(from, to) do
    ...
    case Cache.get_distance(from, to) do
      nil ->
        {lat1, lng1} = get_geolocation(from)
        {lat2, lng2} = get_geolocation(to)
  ...
  defp calculate_distance({lat1, lng1}, {lat2, lng2}) do
    ...
    lng_diff = degrees_to_radians(lng2 - lng1)
    ...
    sin_lng_diff_sq = Math.sin(lng_diff/2) |> Math.pow(2)

    a = sin_lat_diff_sq + (cos_lat1 * cos_lat2 * sin_lng_diff_sq)
